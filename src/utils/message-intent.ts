const GREETING_PATTERN =
  /^(hi|hello|hey|hola|namaste|yo|sup|good\s+(morning|afternoon|evening)|gm|howdy)[\s!.,?]*$/i;

const THANKS_PATTERN =
  /^(thanks|thank\s+you|thx|ty|ok|okay|got\s+it|cool|great)[\s!.,?]*$/i;

/**
 * True when the message is clearly a reminder request (AI or relative parser).
 */
export function looksLikeReminderIntent(text: string): boolean {
